...
class Settings:
    ...
    def check_dataset(self):
        ...
        '''
        files_tp = glob.glob1(self.folder_tampered, '*.tif')
        files_au = glob.glob1(self.folder_authentic, '*')
        files_mask = glob.glob1(self.folder_mask, '*_3.jpg')
        ...
        for i, name in enumerate(files_tp):
            ...
            dataset.append((tmp_file, os.path.join(self.folder_mask, mask_file[-1])))
        return dataset


class DataHandler_Columbia(T.DataHandler):
    def __init__(self):
    ...
    def prepare_data_patches(self, iw, ih, pximage=10):
        ...
        for i, n in enumerate(self.image_db):
            ...
            while True:
                ...
                maxnum = iw * ih
                if np.sum(im2) < 0.1 * maxnum or np.sum(im2) > 0.9 * maxnum:
                    tmp_check += 1
                    if tmp_check % 200 == 0:
...
t1 = time()
data_reader = DataHandler_Columbia()
